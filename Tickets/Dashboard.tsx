...
const Dashboard = ({ navigation }: { navigation: any }) => {
  ...
  const getPriorityTicket = (ticketsArray: any[]) => {
    const high = ticketsArray.find(
      (t: { priority_rank: string }) => t.priority_rank === 'High',
    );
    if (high) return high;

    const medium = ticketsArray.find(
      (t: { priority_rank: string }) => t.priority_rank === 'Medium',
    );
    if (medium) return medium;

    const low = ticketsArray.find(
      (t: { priority_rank: string }) => t.priority_rank === 'Low',
    );
    if (low) return low;
